...
Matrix<Fraction> computeParticularSolution(const Matrix<Fraction>& R, std::size_t n)
{
    ...
    for (std::size_t r = 0; r < R.rows(); ++r) {
        ...
        if (pc < n) {
            gamma(pc, 0) = R(r, n);
            for (std::size_t c = pc + 1; c < n; ++c)
                gamma(pc, 0) -= R(r, c) * gamma(c, 0);
        }
    }
    return gamma;
}
    ...
    : QWidget(parent)
{
    ...
    connect(backBtn, &QPushButton::clicked, this, &NonhomoLinearSystemPage::backRequested);
    topLay->addWidget(backBtn);

    auto* titleLbl = new QLabel(QStringLiteral("解非齐次线性方程组"));
    ...
    connect(genBtn, &QPushButton::clicked, this, &NonhomoLinearSystemPage::onGenerate);
...
void NonhomoLinearSystemPage::onGenerate()
{
    ...
    cells_.resize(static_cast<std::size_t>(curM_ * curN_ + curM_));
    ...
    for (int i = 0; i < curM_; ++i) {
        ...
        auto* bEdit = new QLineEdit;
        bEdit->setFixedWidth(56);
        bEdit->setAlignment(Qt::AlignCenter);
        bEdit->setPlaceholderText(QStringLiteral("0"));
        cells_[static_cast<std::size_t>(curM_ * curN_ + i)] = bEdit;
        bEdit->installEventFilter(this);
        lay->addWidget(bEdit, i, gc++);
    }
...
void NonhomoLinearSystemPage::onSolve()
{
    ...
    for (int i = 0; i < curM_; ++i) {
        ...
        try {
            b(i, 0) = parseFraction(cells_[static_cast<std::size_t>(
                curM_ * curN_ + i)]->text());
        } catch (...) {
            resultBrowser_->setHtml(QStringLiteral(
                "<p style=\"color:#E74C3C; font-size:14px;\">"
    ...
    for (std::size_t i = 0; i < Aug.rows(); ++i) {
        for (std::size_t j = 0; j < static_cast<std::size_t>(curN_); ++j)
            Aug(i, j) = A(i, j);
        Aug(i, static_cast<std::size_t>(curN_)) = b(i, 0);
    }
    ...
    for (std::size_t r = 0; r < R.rows(); ++r) {
        bool allZero = true;
        for (std::size_t c = 0; c < n; ++c) {
            if (!R(r, c).isZero()) { allZero = false; break; }
        }
    ...
    parts << formulaHtml(eqnSysLtx(A, b), th, doc);
    ...
    // General solution formula
    {
        ...
        for (std::size_t ri = 0; ri < R.rows(); ++ri) {
            ...
            if (!R(ri, n).isZero()) {
                bool neg = R(ri, n).sign() < 0;
                Fraction ac = R(ri, n).abs();
                if (first) {
                    if (neg) row += QStringLiteral("-");
                } else {
                    row += neg ? QStringLiteral(" - ") : QStringLiteral(" + ");
            ...
            ++written;
            row += (written < static_cast<int>(pivotCols.size()))
...
void NonhomoLinearSystemPage::onDemo()
{
    ...
    const std::vector<int> demoB = {4, -7, -2};
    for (int i = 0; i < 3; ++i)
        cells_[12 + i]->setText(QString::number(demoB[i]));
